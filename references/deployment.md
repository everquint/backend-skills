# Terraform or OpenTofu and Deployment

## Contents

- Infrastructure as code only when required
...
Do not create, propose, or scaffold infrastructure as code merely because a project is a backend. Add root `terraform` only when infrastructure provisioning is actually required, the user asks for it, or the task changes an existing Terraform-compatible implementation.

When infrastructure as code is required, use Terraform or OpenTofu and define the infrastructure under root `terraform`. Follow an existing compatible tool when one is already selected; otherwise confirm Terraform versus OpenTofu with the user when a channel exists. Do not use AWS CloudFormation, AWS CDK, Ansible, Pulumi, handwritten cloud provisioning scripts, or console-only resources as infrastructure as code without an explicit approved exception and ADR.

Keep infrastructure changes in the same repository and review process as the application version they support. Import existing managed resources into the selected Terraform-compatible state rather than leaving permanent undocumented infrastructure outside state.
...
- Use lowercase snake_case for Terraform-compatible block labels, variables, locals, and outputs. Use kebab-case for directories and filenames when not using the conventional single-word filenames above.
- Declare the selected Terraform or OpenTofu CLI, provider, and module version constraints. Commit each root module's `.terraform.lock.hcl` and update it through reviewed dependency changes.
...
- Prefer secret-manager references and workload identity. Avoid placing secret values in Terraform-compatible variables or resource arguments when the provider would persist them in state.
...
- Run the selected CLI's `fmt -check`, `validate`, and test commands, plus TFLint and an approved infrastructure security scanner, for authored modules in pull requests. Use `terraform` for Terraform and `tofu` for OpenTofu consistently; never mix their state operations casually.
...
- Define health checks, resource requests and limits, replica count, autoscaling bounds, disruption behavior, termination grace, and rollout strategy in Terraform, OpenTofu, or the selected tool's managed platform configuration.
- Use workload identity and least-privilege network and cloud permissions for each deployable REST, gRPC, MCP, and workflow image.
...
- Use `moved` or import declarations when renaming or adopting resources so the selected tool does not destroy and recreate them accidentally.
...
Verify formatting, validation, lint, security policy, module tests, plans for supported environments, state locking, least privilege, private networking, trusted-proxy behavior, rollout health, autoscaling bounds, restore procedures, drift detection, and destructive approval. A green Terraform or OpenTofu command that loaded no configuration or targeted the wrong environment is a failed gate.

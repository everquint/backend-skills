...
2. Create only the `src/api`, `src/debug`, `src/docs`, and `src/dockerfiles` subdirectories the initial scope requires. Create `src/terraform` only when the user explicitly asks for Terraform.

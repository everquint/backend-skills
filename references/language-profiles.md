...
- Organize Rust files as cohesive modules rather than enforcing one struct per file. Keep a domain struct and its inherent `impl` blocks together by default, while allowing closely related structs, enums, traits, request or response types, and private helpers to share a file when that is the clearest idiomatic design. Place a trait `impl` with the type or adapter that owns the integration; split a module only when cohesion or navigability improves.

...
description: Apply Ever Quint's language-neutral backend engineering standards when creating, changing, fixing, testing, debugging, documenting, releasing, observing, deploying, orchestrating, or reviewing backend repositories and features. Use for backend architecture, object-oriented logic, ORM and data integrity, REST/gRPC/MCP/events, production Temporal workflows and payload limits, runtime resilience, memory safety and leak prevention, infrastructure dependencies, application security, trusted proxies and client IPs, secrets, OpenTelemetry/OpenObserve, debug tooling, Terraform or OpenTofu, feature flags, effective tests and coverage, JavaScript/TypeScript and pnpm workspaces, Go, Python, Rust and Cargo workspaces, parallel agent orchestration and worktrees, delivery workflow, documentation, supply-chain security, Docker images, package publishing, or backend code quality.
---

# Ever Quint Backend Standards

Build backend changes around a single object-oriented business-logic core. Treat REST, gRPC, MCP, and workflows as consumers; isolate persistence and external providers behind ORM and service boundaries.
...
- Read [architecture.md](references/architecture.md) before designing or changing repository structure, modules, dependencies, ORM access, services, REST, gRPC, MCP, or workflows.
...
- Read [debugging.md](references/debugging.md) when creating or changing root `debug`, a REST Swagger server, gRPC inspection, MCP Inspector integration, or direct Temporal workflow execution without a Temporal server.
...
- Read [memory-safety.md](references/memory-safety.md) when code creates or retains listeners, timers, tasks, goroutines, threads, caches, queues, buffers, pools, streams, sockets, files, native allocations, reference cycles, or other long-lived resources, and when investigating or verifying memory growth.
- Read [api-and-events.md](references/api-and-events.md) when changing REST, gRPC, or MCP contracts, validation, errors, pagination, idempotency, versioning, rate limits, webhooks, queues, events, scheduled jobs, or CLI consumers.
...
- Read [deployment.md](references/deployment.md) when infrastructure as code is required, the user asks for Terraform or OpenTofu, or the task changes an existing root `terraform` tree. Do not infer or scaffold infrastructure as code from a general backend request.
...
- When initializing or changing language-specific structure or tooling, read the shared [language profile router](references/language-profiles.md), then load only the matching profile: [JavaScript/TypeScript](references/language-javascript-typescript.md), [Go](references/language-go.md), [Python](references/language-python.md), or [Rust](references/language-rust.md). For polyglot changes, load only the languages whose authored code or tooling changes. Do not read unselected profiles. The Rust profile contains the stable Rust Book and `rust-skills` prerequisites for non-trivial Rust work.
- When strengthening mechanical enforcement, read only the matching language reference: [mechanical-checks-javascript-typescript.md](references/mechanical-checks-javascript-typescript.md), [mechanical-checks-go.md](references/mechanical-checks-go.md), [mechanical-checks-python.md](references/mechanical-checks-python.md), or [mechanical-checks-rust.md](references/mechanical-checks-rust.md). Ask the user to confirm any new framework, middleware, or test dependency before installation; the required behavior and coverage thresholds are not optional.
...
- Never let REST, gRPC, MCP, Temporal workflows, Temporal activities, or other consumers access the database, ORM, or external services directly.
...
- Treat retained-memory and resource leaks as release-blocking defects. Give every acquired resource an owner and bounded lifetime, and verify leak-prone paths with repeated lifecycle tests or profiling before claiming them leak-free.
